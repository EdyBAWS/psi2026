...
export async function seedDosareDauna(prisma: PrismaClient) {
  ...
  const inspectorByCnp = new Map(
    (await prisma.angajat.findMany({ where: { tipAngajat: 'Inspector' } })).map(a => [a.CNP, a])
  );
  ...
  const data = [
    {
      ...
      idAsigurator: getRequired(asigByCui, 'RO6120740', 'asigurator').idAsigurator,
      idInspector: getRequired(inspectorByCnp, '1800101229988', 'inspector').idAngajat,
      status: StatusGeneral.Activ,
    },
    {
      numarDosar: 'DAUNA-2026-002',
      idClient: getRequired(clientByCnp, '2870306123456', 'client CNP').idClient,
      idVehicul: getRequired(vehiculByNr, 'CJ-12-AUT', 'vehicul').idVehicul,
      idAsigurator: getRequired(asigByCui, 'RO5587260', 'asigurator').idAsigurator,
      idInspector: getRequired(inspectorByCnp, '1800101229988', 'inspector').idAngajat,
      status: StatusGeneral.Activ,
    },
    {
      ...
      idAsigurator: getRequired(asigByCui, 'RO336290', 'asigurator').idAsigurator,
      idInspector: getRequired(inspectorByCnp, '2850515229988', 'inspector').idAngajat,
      status: StatusGeneral.Activ,
    },
    {
      ...
      idAsigurator: getRequired(asigByCui, 'RO6291812', 'asigurator').idAsigurator,
      idInspector: getRequired(inspectorByCnp, '2850515229988', 'inspector').idAngajat,
      status: StatusGeneral.Activ,
    },
    {
      ...
      idAsigurator: getRequired(asigByCui, 'RO2884407', 'asigurator').idAsigurator,
      idInspector: getRequired(inspectorByCnp, '1800101229988', 'inspector').idAngajat,
      status: StatusGeneral.Activ,
    },
    {
      numarDosar: 'DAUNA-2026-006',
      idClient: getRequired(clientByCnp, '1850822223344', 'client CNP').idClient,
      idVehicul: getRequired(vehiculByNr, 'CT-33-DNG', 'vehicul').idVehicul,
      idAsigurator: getRequired(asigByCui, 'RO9457880', 'asigurator').idAsigurator,
      idInspector: getRequired(inspectorByCnp, '2850515229988', 'inspector').idAngajat,
      status: StatusGeneral.Activ,
    },
    {
      ...
      idAsigurator: getRequired(asigByCui, 'RO1813613', 'asigurator').idAsigurator,
      idInspector: getRequired(inspectorByCnp, '1800101229988', 'inspector').idAngajat,
      status: StatusGeneral.Activ,
    },
  ];
  ...
  for (const item of data) {
    await prisma.dosarDauna.create({ data: item });
  }
