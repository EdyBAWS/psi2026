...
export default function PreluareAutoContext({
  clientSelectat, comandaActivaExistenta, esteLucrareAsigurare, onSchimbaFluxAsigurare, vehiculSelectat,
    ...
    <div className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
      <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
        ...
        <dl className="mt-5 grid gap-4 text-sm text-slate-600 md:grid-cols-2">
          ...
          <div><dt className="font-semibold text-slate-700">Serie șasiu</dt><dd className="mt-1 break-all">{vehiculSelectat.vin}</dd></div>
      ...
      <div className="flex flex-col rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
        <div className="mb-5 flex items-center justify-between gap-3">
          <div>
            ...
            <h3 className="mt-2 text-xl font-bold text-slate-800">{esteLucrareAsigurare ? "Lucrare cu asigurare" : "Lucrare client / flotă"}</h3>
          </div>
        ...
        {comandaActivaExistenta && (
          <div className="mb-4 flex-1 rounded-xl border border-rose-200 bg-rose-50 px-4 py-4 text-sm text-rose-800">
            Există deja comanda activă <strong>{comandaActivaExistenta.numarComanda}</strong> pentru acest vehicul, cu status <strong>{comandaActivaExistenta.status}</strong> și termen promis <strong>{formatData(comandaActivaExistenta.termenPromis)}</strong>.
          </div>
        )}
      </div>
    </div>
