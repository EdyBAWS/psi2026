// src/modules/01entitati/asigurator/Asigurator.tsx
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-left text-sm">
                <thead className="bg-slate-50 text-[10px] font-bold uppercase tracking-wider text-slate-400 border-b border-slate-200">
                  <tr><th className="px-6 py-4">Denumire & CUI</th><th className="px-6 py-4">Telefon Contact</th><th className="px-6 py-4">Status</th><th className="px-6 py-4 text-center">Acțiuni</th></tr>
                </thead>
                  ...
                  {listaFiltrata.map(asig => (
                    <tr key={asig.idAsigurator} className={`hover:bg-slate-50 transition-colors group ${asig.status === 'Inactiv' ? 'opacity-60' : ''}`}>
                      <td className="px-6 py-4 font-bold text-slate-800">
                        {asig.denumire} <br/>
                        <span className="text-[10px] font-mono text-slate-500">CUI: {asig.CUI}</span>
                      </td>
