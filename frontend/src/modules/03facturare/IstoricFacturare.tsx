...
export default function IstoricFacturare() {
  ...
  const tranzactiiFiltrate = useMemo(() => {
    return istoricMock.filter(trx => {
      const matchSearch = trx.client.toLowerCase().includes(searchTerm.toLowerCase()) || trx.numarDocument.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
      <div className="flex justify-between items-end mb-8 border-b border-slate-100 pb-6">
        ...
        <div className="text-right">
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Documente Emise</p>
        ...
        <table className="min-w-full bg-white text-left text-sm whitespace-nowrap">
          ...
          <tbody className="divide-y divide-slate-100">
            {tranzactiiFiltrate.length === 0 ? (
              <tr><td colSpan={7} className="py-8 text-center text-slate-500">Nu a fost găsită nicio înregistrare.</td></tr>
            ) : (
              tranzactiiFiltrate.map((trx) => (
                <tr key={trx.id} className="hover:bg-slate-50 transition-colors">
                  <td className="py-4 px-6 text-slate-500 font-medium">{trx.dataOra}</td>
                  <td className="py-4 px-6"><span className={`px-3 py-1 rounded-full text-xs font-bold border ${getBadgeColor(trx.tipOperatiune)}`}>{trx.tipOperatiune}</span></td>
                  ...
                  <td className={`py-4 px-6 text-right font-bold ${trx.valoare < 0 ? 'text-rose-600' : 'text-emerald-600'}`}>{trx.valoare > 0 ? '+' : ''}{trx.valoare.toFixed(2)}</td>
