...
// Interfețe pentru a simula baza de date
interface Client {
  idClient: number;
  nume: string;
  soldDebitor: number;
...
const Penalizare: React.FC = () => {
  // --- DATE DE TEST (Mock Data simulate din JOIN-uri) ---
  ...
  // State pentru Searchable Dropdown (Client)
  const [searchTermClient, setSearchTermClient] = useState('');
  const [isClientDropdownOpen, setIsClientDropdownOpen] = useState(false);
  const clientDropdownRef = useRef<HTMLDivElement>(null);

  // Parametri pentru calcul penalizare
  const [dataCalcul, setDataCalcul] = useState<string>(new Date().toISOString().split('T')[0]); // Data de azi
  const [procentPenalizare, setProcentPenalizare] = useState<number>(1); // Pornim cu 1% număr întreg
  ...
  // Handler selecție client din search
  const handleSelectClient = (c: Client) => {
    setIdClientSelectat(c.idClient);
    setSearchTermClient(`${c.nume} (CUI: ${c.CUI})`);
    setIsClientDropdownOpen(false);
    setIdFacturaSelectata(''); // Resetăm factura
  };

  // CALCULE INTELIGENTE (Business Logic Real)
  ...
  return (
    <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 max-w-5xl mx-auto">
      ...
      <form onSubmit={handleGenereazaPenalizare} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Partea Stângă: Selecție și Parametri */}
        <div className="lg:col-span-2 space-y-8">
          ...
          {/* SECȚIUNEA 2: Parametrii Calculului Auto */}
        ...
        {/* Partea Dreaptă: Sumar Financiar și Acțiune */}
