...
function numeClientDinVehicul(idVehicul: number): string {
  ...
  return idClient ? (clientiById.get(idClient) ?? `Client #${idClient}`) : 'Client necunoscut';
}
...
export function obtineLiniiFacturaDinComandaMock(idComanda: number): LinieFacturaMock[] {
  return mockPozitii
...
export function obtineComenziFacturabileDinMock(): ComandaFacturabilaMock[] {
  ...
  return mockComenzi
    .filter(
      (comanda) =>
        statusuriFacturabile.has(comanda.status) && !comenziDejaFacturate.has(comanda.idComanda),
    )
    .map((comanda) => ({
      idComanda: comanda.idComanda,
      nrComanda: comanda.nrComanda,
      dataComanda: (comanda.dataFinalizare ?? comanda.dataDeschidere).toISOString().split('T')[0],
      idVehicul: comanda.idVehicul,
      status: comanda.status,
      totalEstimat: comanda.totalEstimat,
      client: numeClientDinVehicul(comanda.idVehicul),
      vehicul: vehiculeById.get(comanda.idVehicul) ?? `Vehicul #${comanda.idVehicul}`,
    }));
}

export const facturiEmiseMock: FacturaMock[] = facturiEmiseSeed.map((facturaSeed) => {
  if (facturaSeed.idComanda !== null) {
    const comanda = mockComenzi.find((item) => item.idComanda === facturaSeed.idComanda);
    if (!comanda) {
      throw new Error(`Comanda ${facturaSeed.idComanda} nu există în mock-ul operațional.`);
    }

    const idClient = vehiculLaClient.get(comanda.idVehicul);
    if (!idClient) {
      throw new Error(`Vehiculul ${comanda.idVehicul} nu are client în mock-ul operațional.`);
    }
  ...
  return {
    ...
    dataEmitere: facturaSeed.dataEmitere,
    dataScadenta: facturaSeed.dataScadenta,
    client: clientiById.get(facturaSeed.idClient) ?? `Client #${facturaSeed.idClient}`,
...
export function obtineLiniiFacturaEmisaDinMock(idFactura: number): LinieFacturaMock[] {
...
export const istoricFacturareMock: TranzactieIstoricMock[] = [
  ...
  },
  {
    ...
    utilizator: 'Edy (Admin)',
    detalii: 'Facturare comandă CMD-2026-006 pentru lucrare suspensie și bujii.',
  },
  {
    id: 'TRX-006',
